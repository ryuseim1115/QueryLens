...
def test_simple_select_returns_rows():
    result = run_subqueries([_make("SELECT id, name FROM test1 WHERE id = 1")])
    assert len(result[0].result) == 1
    assert result[0].result[0]["name"] == "山田太郎"


def test_aggregate_returns_count():
    result = run_subqueries([_make("SELECT COUNT(*) AS cnt FROM test1")])
...
def test_multiple_subqueries_execute_independently():
    subqueries = [
        _make("SELECT COUNT(*) AS cnt FROM test1"),
        _make("SELECT COUNT(*) AS cnt FROM test3"),
    ]
    result = run_subqueries(subqueries)
    assert result[0].result[0]["cnt"] == 5
    assert result[1].result[0]["cnt"] == 5


def test_join_query_returns_results():
    query = (
        "SELECT u.name, o.amount "
        "FROM test1 AS u JOIN test2 AS o ON u.id = o.user_id "
